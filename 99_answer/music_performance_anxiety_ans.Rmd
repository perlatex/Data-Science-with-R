...
```{r}
music_lev2 %>%
  ggplot(aes(x = meanbysubj)) +
  geom_histogram(binwidth = 2, color = "white") +
  xlab("Mean Negative Affect") +
  ylab("Frequency") 
```
...
```{r}
music_lev2 %>%
  ggplot(aes(x = gender, y = meanbysubj)) +
  geom_boxplot() +
  geom_jitter() +
...
```{r}
music_lev2 %>%
  t.test(meanbysubj ~ gender, data = .)
```

11. 负性情绪越严重的人，越容易出现表演焦虑？
```{r}
music_lev2 %>%
  ggplot(aes(x = mpqnem, y = meanbysubj)) +
  geom_point() +
...
```{r}
music_lev2 %>%
  ggplot(aes(x = mpqsr, y = meanbysubj)) +
  geom_point() +
  geom_smooth(method = "lm")
```
...
```{r}
music_lev2 %>%
  ggplot(aes(x = years_study, y = meanbysubj)) +
  geom_point() +
  geom_smooth(method = "lm")
```
...
```{r}
tb %>% 
  mutate(orch = as.factor(orch)) %>% 
  ggplot(aes(x = mpqnem_c, y = meanbysubj, colour = orch)) +
  geom_point(aes(shape = orch)) +
  geom_smooth(aes(linetype = orch), method = "lm", se = FALSE)
```
...
$$
```{r}
mod <- lm(meanbysubj ~ 1 + mpqnem_c + orch + mpqnem_c:orch, data = tb) 
broom::tidy(mod)
```

$$
\text{meanbysubj}_{i}=16.13+0.17\textrm{mpqnem_c}_{i}+0.525\textrm{orch}_{i}-0.083\textrm{mpqnem_c}_{i}\times\textrm{orch}_{i}
$$
...
\begin{align*}
 \textrm{orch}=0: & \\
&\text{meanbysubj}_{i}=16.13+0.17\textrm{mpqnem_c}_{i}\\
 \textrm{orch}=1: & \\
&\text{meanbysubj}_{i} = (16.13+0.525)+(0.17-0.083)\textrm{mpqnem_c}_{i}
 \end{align*}
$$