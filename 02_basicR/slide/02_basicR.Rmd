...
**对象的创建与使用**。首先确定一个名称，然后使用赋值操作符 `<-`，(在Rstudio中同时按下`alt` 和 `-`，就可以产生赋值箭头)，将数据赋值给它。
...
可以把 x 想象成一个**盒子**，我们把5这个数放入了盒子。当键入 `x` 然后回车，就打印出 `x` 的值。
```{r print x}
x
```
...
数据框就是我们经常用的excel表格，所以在统计建模和可视化中运用非常广泛。
